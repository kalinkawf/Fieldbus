#include <iostream>
#include <iomanip>
#include <sstream>
#include <vector>
#include <string>
#include <chrono>


// Funkcja do obliczania CRC-16
uint16_t getTableCRC(const std::vector<uint8_t>& data) {
    static unsigned char aCRCHi[] =
    {
    
    };
    static unsigned char aCRCLo[] =
    {
    
    };
    uint8_t crc_low = 0xFF;
    uint8_t crc_high = 0xFF;
    uint8_t index;
    for (uint8_t byte : data) {
        index = crc_high ^ byte;
        crc_high = crc_low ^ aCRCHi[index];
        crc_low = aCRCLo[index];
    }

    return crc_high << 8 | crc_low;
}

bool isValidHexChar(char c) {
    return ((c >= '0' && c <= '9') || (c >= 'A' && c <= 'F') || (c >= 'a' && c <= 'f'));
}

int hexCharToInt(char c) {
    if (isdigit(c)) {
        return c - '0';
    } else {
        return std::tolower(c) - 'a' + 10;
    }
}

uint8_t hexToByte(const std::string& hexByte) {
    return static_cast<uint8_t>((hexCharToInt(hexByte[0]) << 4) | hexCharToInt(hexByte[1]));
}

int main() {
    std::vector<uint8_t> byteSequence;
    std::string input;

    std::cout << "Podaj sekwencje bajtow w notacji hex: ";
    std::getline(std::cin, input);

    if (input.size() % 2 != 0) {
        input.insert(input.size(), 1, '0');
    }

    for (size_t i = 0; i < input.size(); i += 2) {
        std::string hexByte = input.substr(i, 2); // Pobierz dwa znaki szesnastkowe

        // Konwersja dwóch znaków szesnastkowych na wartość bajtową
        uint8_t byteValue = hexToByte(hexByte);

        // Dodaj wartość bajtową do wektora
        byteSequence.push_back(byteValue);
    }

    if (byteSequence.size() > 256) {
        std::cerr << "Dlugosc sekwencji przekracza 256 bajtow." << std::endl;
        return 1;
    }

    // pobieranie liczby powtórzeń algorytmu
    uint32_t n;
    std::cout << "Podaj liczbe n powtorzen algorytmu CRC (1 do 1000000000): ";
    std::cin >> n;

    if (n < 1 || n > 1000000000) {
        std::cerr << "Wartosc poza zakresem." << std::endl;
        return 1;
    }

    auto startTime = std::chrono::high_resolution_clock::now(); // Algorytm start

    // std::vector<uint8_t> newSequence;
    // newSequence.push_back(0x01);
    // newSequence.push_back(0x10);
    // newSequence.push_back(0x0);
    // newSequence.push_back(0x0);
    // newSequence.push_back(0x0);
    // newSequence.push_back(0x01);
    // newSequence.push_back(0x02);
    // newSequence.push_back(0x0);
    // newSequence.push_back(0x11); // 01 10 00 00 00 01 02 00 11

    uint16_t crcResult = getTableCRC(byteSequence);
    for (unsigned int i = 0; i < n-1; ++i) {
        getTableCRC(byteSequence);
    }

    auto endTime = std::chrono::high_resolution_clock::now(); // Algorytm koniec
    std::chrono::duration<double> duration = std::chrono::duration_cast<std::chrono::duration<double>>(endTime - startTime);

    std::cout << "Suma CRC po " << n << " powtorzeniach: 0x" << std::hex << crcResult << std::dec << std::endl;
    std::cout << "Czas: " << duration.count() << "s" << std::endl;
    system("pause");
    return 0;
}
